...
if (!empty($class_attendance_data)):
    ...
    <div class="m3-att-block shadow-sm">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h6 class="fw-black m-0" style="font-size: 0.9rem; color: #6750A4;"><i
                    class="bi bi-calendar2-check-fill me-2"></i>Attendance Status</h6>
        ...
        <?php foreach ($class_attendance_data as $att): ?>
            <div class="att-card shadow-smx">
                <div class="d-flex justify-content-between align-items-start mb-1">
                    <div>
                        <span class="fw-black fw-bold text-dark" style="font-size: 0.85rem;"><?= $att['classname'] ?> -
                            <?= $att['sectionname'] ?></span>
                        <div class="mt-1">
                            <?php if ($att['is_synced']): ?>
                                <span class="sync-badge is-synced badge-success"><i
                                        class="bi bi-check-circle-fill me-1"></i>Synced</span>
                            <?php else: ?>
                                <span class="sync-badge not-synced badge-warning"><i class="bi bi-cloud-slash-fill me-1"></i>Not
                                    Synced</span>
                            <?php endif; ?>
                        </div>
                    </div>
                    <div class="text-end">
                        <div class="fw-black" style="font-size: 1.2rem; color: #6750A4; line-height:1;">
                            <?= $att['effective_rate'] ?>%
                        </div>

                        <a class="btn btn-link ms-3 fs-5 text-danger"
                            href="stattnd.php?cls=<?= $att['classname'] ?>&sec=<?= $att['sectionname'] ?>&year=<?= $sessionyear ?>"><i
                                class="bi bi-fingerprint"></i></a>

                        <a class="btn btn-link ms-3 fs-5 text-success"
                            href="st-attnd-register.php?cls=<?= $att['classname'] ?>&sec=<?= $att['sectionname'] ?>"><i
                                class="bi bi-pencil-square"></i></a>


                        <?php if (!$att['is_synced'] && $att['total'] >= 0): ?>
                            <button class="sync-btn mt-2"
                                onclick="syncNow('<?= $att['classname'] ?>', '<?= $att['sectionname'] ?>', this)">
