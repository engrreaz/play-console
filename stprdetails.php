...
<style>
    ...
    .st-avatar { width: 64px; height: 64px; border-radius: 20px; object-fit: cover; margin-right: 16px; border: 2px solid #EADDFF; }
    
    .paid-badge {
        background: #E8F5E9; color: #2E7D32; padding: 12px 20px;
        border-radius: 20px; text-align: center; margin: 0 16px 20px;
    }
    ...
    .pr-card:active { transform: scale(0.98); background-color: #F3EDF7; }
    ...
    .pr-amount { font-weight: 800; font-size: 1.1rem; color: #146C32; text-align: right; }
    .breakdown-box { background: #F7F2FA; border-radius: 16px; padding: 12px; margin-top: 12px; display: none; }
    .breakdown-item { display: flex; justify-content: space-between; font-size: 0.75rem; padding: 4px 0; border-bottom: 1px dashed #E7E0EC; }
</style>
...
<main class="pb-5 mt-2">
    <div class="st-hero-card shadow-sm">
        <img src="<?php echo $photo_path; ?>" class="st-avatar shadow-sm" onerror="this.src='https://eimbox.com/students/noimg.jpg';">
        ...
        ?>
            <div class="pr-card shadow-sm" onclick="toggleBreakdown('<?php echo $prno; ?>')">
                ...
                <div class="breakdown-box shadow-sm" id="hde<?php echo $prno; ?>">
                    ...
                    <?php endwhile; $stmt_item->close(); ?>
                    
                    <div class="text-center mt-3">
                        <button class="btn btn-sm btn-primary rounded-pill px-4" onclick="event.stopPropagation(); epos('<?php echo $prno; ?>')">
...
<script>
    ...
    function epos(prno) {
        window.location.href = "stpr.php?prno=" + prno;
    }
</script>
