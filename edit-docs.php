...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['save_doc'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO page_docs (pagename, title, description, tips, notes) 
                            VALUES (?, ?, ?, ?, ?) 
                            ON DUPLICATE KEY UPDATE 
                            title=VALUES(title), description=VALUES(description), 
                            tips=VALUES(tips), notes=VALUES(notes)");
    $stmt->bind_param("sssss", $p_name, $title, $desc, $tips, $notes);
...
<body style="background:#f8f9fa;">
    <main class="container py-5">
        <div class="card border-0 shadow-sm rounded-4 p-4">
            <h4 class="fw-bold text-primary mb-4">Edit Documentation for: <span class="text-dark"><?= $page_to_edit ?></span></h4>
            ...
            <form action="" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="title" class="form-control rounded-3" value="<?= $data['title'] ?? $page_to_edit ?>" required>
                </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <textarea name="notes" class="form-control rounded-3" rows="4"><?= $data['notes'] ?? '' ?></textarea>
                    </div>
                </div>

                <div class="d-flex gap-2 mt-4">
                    <button type="submit" name="save_doc" class="btn btn-primary px-5 rounded-pill fw-bold shadow">Save & Return</button>
