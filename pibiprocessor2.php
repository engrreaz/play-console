    ...
    */
    ...
    // reset or previous process.................................................
        ...
        if($tail == 0 ){
            ...
            $pibicountos = array();
            $sql242fra = "SELECT subcode, assesstype, count(*) as cntos FROM pibientry where sessionyear='$sy' and sccode='$sccode' and classname='$clsf' and sectionname = '$secf' and exam ='$exam' and assessment>0 group by subcode, assesstype order by subcode, assesstype;"; 
            //echo $sql242fra;
            $result242frax = $conn->query($sql242fra); if ($result242frax->num_rows > 0) {while($row242fra = $result242frax->fetch_assoc()) {
            $pibicountos[] = $row242fra; 
            }}
            
            //echo var_dump($pibicountos);         
            
            
            $datayn = array();
            $sql242frax = "SELECT id, subcode, assess FROM pibiprocess where sessionyear='$sy' and classname='$clsf' and sectionname = '$secf' and exam ='$exam' and sccode = '$sccode'"; 
            $result242frax = $conn->query($sql242frax); if ($result242frax->num_rows > 0) {while($row242frax = $result242frax->fetch_assoc()) { 
                $datayn[] = $row242frax;
            }}
            
            //echo var_dump($datayn);  
            
            //*****************************************************************************************************************************************************            
            $sql242 = "SELECT * FROM subsetup where classname='$clsf' and sectionname = '$secf' and sccode='$sccode' and sessionyear='$sy' order by subject"; 
            // echo $sql242 ;
            ...
            $en = 0; if($ss>0) $en++; if($oo>0) $en++; if($pp>0) $en++;
                    ...
                    //echo '**'.$stcount.'***<br>';
                        ...
                        $txt = $assesstype . ' for ' . $subname . ' on class ' . $clsf . ' | ' . $secf;
                            ...
                            $pibidone = 0;
                            foreach($pibicountos as $pbc){
                                if($pbc["subcode"] == $subcode && $pbc["assesstype"] == $assesstype){
                                    $pibidone = $pbc['cntos'];
                                }
                            }
                            ...
                            $lid = 0;
                            foreach($datayn as $dyn){
                                // echo '<br>---------------------------------<br>';
                                // echo $dyn["subcode"] . ' * ' . $dyn["assess"] . ' * ' . $dyn['id'];
                                
                                if($dyn["subcode"] == $subcode && $dyn["assess"] == $assesstype){
                                    $lid = $dyn['id'];
                                }
                            }
                        ...
                        if($lid>0){
                            $query3 ="UPDATE pibiprocess set jobdone='$pibidone', jobreq='$req', jobrate='$rate', sccategory='$sctype' where id='$lid';";
                        } else {
                            $query3 ="insert into pibiprocess (id, sessionyear, sccategory, sccode, exam, classname, sectionname, subcode, subname, assess, txt, jobdone, jobreq, jobrate, status, jobby, jobdate) values 
                            (NULL, '$sy', '$sctype', '$sccode', '$exam', '$clsf','$secf','$subcode', '$subname', '$assesstype', '$txt', '$pibidone', '$req', '$rate', '$sta', '$usr', '$cur')";
                        }
                           echo $query3; 
                        ...
                        */
                        ...
                        $query3 ="insert into pibiprocess (id, sessionyear, sccategory, sccode, exam, classname, sectionname, subcode, subname, assess, txt, jobdone, jobreq, jobrate, status, jobby, jobdate) values 
                        (NULL, '$sy', '$sctype', '$sccode', '$exam', '$clsf','$secf','$subcode', '$subname', '$assesstype', '$txt', '$entrydone', '$req', '$rate', '$sta', '$usr', '$cur')";
                        $conn->query($query3) ;
            ...
            ?>
            <div style="text-align:center; margin-bottom:8px; border-radius:5px; background:var(--darker); color:var(--lighter);padding:5px; font-size:16px; font-weight:600;">Progress Report</div>
            ...
            $tjob = $tjob + $req; $tdon = $tdon + $done;          
                        ...
                        if($clsf=='Six' || $clsf=='Seven'){
                           // echo $subname . '/' . $sub1 . '/' . $newsub . '/';
                           if($newsub - $sub1 !=0 || $assess=='Merged BI'){
                               ?>
                               </div>
                                </div>
                               ...
                               <div class="card text-left" style="background:var(--lighter); color:var(--dark);"  >
                                    <div class="card-body">
                                        <table class="table table-zebra" style="width:100%">
                                            <tr>
                                                ...
                                                <td style="color:<?php echo $bcc;?>">
                                                    <h3><b><?php echo $subname ;?></b></h3><span style="font-size:10px; font-style:italic; color:<?php echo $bcc;?>;"><?php echo $txtt;?></span>
                                                </td>
                                            </tr>
                                        </table>
                                        
                                        <?php include 'pibiprocessor2box.php'; ?>
                     
                               <?php
                           } else {
                                include 'pibiprocessor2box.php';
                           }
