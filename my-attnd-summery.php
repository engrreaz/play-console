...
<style>
    ...
    /* Glassmorphic Stat Item for Hero */
    .hero-stat-item {
...
<main class="pb-5">
    <div class="hero-container">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h5 class="fw-black m-0">Attendance Overview</h5>
                <p class="small m-0 opacity-75"><?= date('D, d M Y') ?></p>
            </div>
        ...
        <div class="row g-2">
            <div class="col-4">
                <div class="hero-stat-item">
                    <div class="hero-stat-count" id="c1">0</div>
                    <div class="hero-stat-label">Present</div>
                </div>
            </div>
            <div class="col-4">
                <div class="hero-stat-item">
                    <div class="hero-stat-count" id="c2">0</div>
                    <div class="hero-stat-label">Leave</div>
                </div>
            </div>
            <div class="col-4">
                <div class="hero-stat-item" style="border-color: rgba(255, 100, 100, 0.4);">
                    <div class="hero-stat-count text-warning" id="c3">0</div>
                    <div class="hero-stat-label">Absent</div>
                </div>
            </div>
            <div class="col-4">
                <div class="hero-stat-item">
                    <div class="hero-stat-count" id="c4">0</div>
                    <div class="hero-stat-label">Weekend</div>
                </div>
            </div>
            <div class="col-4">
                <div class="hero-stat-item">
                    <div class="hero-stat-count" id="c5">0</div>
                    <div class="hero-stat-label">Holiday</div>
                </div>
            </div>
            <div class="col-4">
                <div class="hero-stat-item hero-stat-total">
                    <div class="hero-stat-count" id="c6">0</div>
    ...
    <div class="px-1 mt-4">
        <h6 class="ms-4 mb-3 text-secondary fw-bold small text-uppercase" style="letter-spacing: 1px;">Daily Activity
            Log</h6>
        ...
        for ($x = $val_to; $x >= $val_from; $x -= $step):
            ...
            if (str_contains($wday_text, $bar)) {
                $status_title = "Weekend";
                $status_sub = "Weekly Break ($bar)";
                $status_class = "tone-weekend";
                $status_icon = "bi-calendar2-week";
                $c4++;
                $workday_flag = 0;
            } else {
                ...
                if ($workday_flag == 0) {
                    $status_title = "Holiday";
                    $status_sub = "Institutional Holiday";
                    $status_class = "tone-holiday";
                    $status_icon = "bi-sun-fill";
                    $c5++;
                } else {
                    ...
                    if ($att_ind !== false) {
                        ...
                        $in_time = $att_data['realin'];
                        $out_time = $att_data['realout'];
                        $c1++;
                    } else {
                        // ৪. লিভ এপ্লিকেশন চেক
                        $leave_flag = 0;
                        ...
                        if ($leave_flag) {
                            $status_title = "On Leave";
                            $status_sub = "Authorized Leave";
                            $status_class = "tone-leave";
                            $status_icon = "bi-bookmark-star-fill";
                            $c2++;
                        } else {
                            $c3++; // Absent
                        }
            ...
            <div class="m3-log-card shadow-sm">
                ...
                <?php if ($in_time): ?>
                    <div class="text-end ms-2">
                        ...
                        <?php if ($out_time): ?>
                            <div class="m3-time-pill">OUT: <?php echo $out_time; ?></div><?php endif; ?>
                    </div>
                <?php endif; ?>
            </div>
        <?php endfor; ?>
