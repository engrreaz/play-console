...
$dt = date('Y-m-d H:i:s');;
	...
	$sql00 = "SELECT count(*) as ccnntt FROM pibientry where sessionyear='$sy' and exam='$exam' and subcode='$subcode' and classname='$classname' and sectionname='$sectionname' and assesstype='$assess'  and sccode='$sccode'"; 
    //echo $sql00;
    $result00 = $conn->query($sql00);
    if ($result00->num_rows > 0) {while($row00 = $result00->fetch_assoc()) {
    $mot=$row00["ccnntt"];
    }}
    if($assess == 'Merged BI'){$mot = $mot * 10;}
    echo $mot;
    ...
    //echo $rollno;
        ...
        if($ch==1){   //PI Merging
            ...
            $topicid=$row00x["id"];	
                    ...
                    $sql00 = "SELECT * FROM pibientry where sessionyear='$sy' and exam='$exam' and subcode='$subcode' and topicid='$topicid' and stid='$stid' and (assesstype='Continious Assessment' or assesstype = 'Total Assessment')   order by assessment desc LIMIT 1"; 
                    $result00 = $conn->query($sql00);
                    if ($result00->num_rows > 0) {while($row00 = $result00->fetch_assoc()) {
                    $assessment=$row00["assessment"];	
                    
                        $query330 ="INSERT into pibientry (sessionyear, exam, subcode, sccode, classname, sectionname, stid, roll, assesstype, topicid, assessment, entryby, entrytime) 
                        VALUES ('$sy', '$exam', '$subcode', '$sccode', '$classname', '$sectionname', '$stid', '$rollno', 'Merged PI', '$topicid', '$assessment', 'System-AUTO', '$dt')";
            			$conn->query($query330);            
            ...
            $topicid=$row00x["id"];	
                    ...
                    $sql00 = "SELECT assessment, count(assessment) as ass FROM pibientry where sessionyear='$sy' and exam='$exam' and topicid='$topicid' and stid='$stid' and assesstype='Behavioural Assessment'  order by assessment desc, ass desc LIMIT 1"; 
                    $result00 = $conn->query($sql00);
                    if ($result00->num_rows > 0) {while($row00 = $result00->fetch_assoc()) {
                    $assessment=$row00["assessment"];	
                    
                        $query330 ="INSERT into pibientry (sessionyear, exam, subcode, sccode, classname, sectionname, stid, roll, assesstype, topicid, assessment, entryby, entrytime) 
                        VALUES ('$sy', '$exam', '$subcode', '$sccode', '$classname', '$sectionname', '$stid', '$rollno', 'Merged BI', '$topicid', '$assessment', 'System-AUTO', '$dt')";
            			$conn->query($query330);            
                    $tr++;
                    }}
