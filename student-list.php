...
<style>
    ...
    .m3-tab-bar { 
        ...
        padding: 8px 12px; border-bottom: 1px solid #E7E0EC; 
        position: sticky; top: 56px; z-index: 1040;
    }
    .nav-pills .nav-link {
        border-radius: 8px; border: 1px solid #79747E; color: #49454F;
        padding: 6px 16px; font-size: 0.75rem; font-weight: 700; margin-right: 8px;
        transition: 0.2s;
    }
    .nav-pills .nav-link.active { background-color: #6750A4 !important; color: #fff !important; border-color: #6750A4; }

    /* Condensed Summary Card */
    .summary-chip-row { display: flex; gap: 8px; padding: 12px; }
    .m3-stat-chip { 
        flex: 1; background: #F3EDF7; border-radius: 8px; padding: 8px; 
        text-align: center; border: 1px solid #EADDFF; 
    }
    .stat-val { font-size: 1.1rem; font-weight: 800; color: #6750A4; display: block; }
    .stat-lbl { font-size: 0.55rem; font-weight: 700; color: #49454F; text-transform: uppercase; }

    /* Expandable Student Card */
    .st-card {
        background: #fff; border-radius: 8px; padding: 10px 12px; margin: 0 12px 8px;
        border: 1px solid #eee; transition: 0.3s cubic-bezier(0.4, 0, 0.2, 1);
        position: relative; overflow: hidden;
    }
    .st-card:active { background: #F7F2FA; }
    .st-card.expanded { border-color: #6750A4; }
    
    .st-avatar { width: 44px; height: 44px; border-radius: 8px; object-fit: cover; border: 1px solid #E7E0EC; }
    .due-badge { font-size: 0.65rem; font-weight: 800; color: #B3261E; background: #FFEBEE; padding: 2px 6px; border-radius: 4px; }

    /* Action Grid - Appears on Click */
    .action-grid {
        display: none; grid-template-columns: repeat(4, 1fr); gap: 8px;
        margin-top: 12px; padding-top: 12px; border-top: 1px dashed #EADDFF;
    }
    .st-card.expanded .action-grid { display: grid; }
    
    .act-item { text-align: center; color: #49454F; text-decoration: none !important; transition: 0.2s; }
    .act-item i { font-size: 1.4rem; display: block; color: #6750A4; }
    .act-item span { font-size: 0.55rem; font-weight: 800; text-transform: uppercase; margin-top: 4px; display: block; }
    .act-item:active { transform: scale(0.9); }
...
<header class="m3-app-bar">
    ...
    <div class="action-icons"><i class="bi bi-search fs-5"></i></div>
</header>

<main class="pb-5">
    <?php if ($count_class > 0): ?>
    <div class="m3-tab-bar scroll-hide">
            ...
            ?>
                <li class="nav-item">
                    <button class="nav-link <?php echo ($i == 0) ? 'active' : ''; ?>" 
                            ...
                            data-bs-toggle="pill" 
                            data-bs-target="#pane-<?php echo $i; ?>">
        ...
        <div class="tab-pane fade <?php echo ($h == 0) ? 'show active' : ''; ?>" id="pane-<?php echo $h; ?>">
            ...
            <div class="summary-chip-row">
                <div class="m3-stat-chip shadow-sm">
                    <span class="stat-lbl">Class Strength</span>
            ...
            <div class="student-list px-1">
                ...
                while ($row = $res_s->fetch_assoc()):
                    ...
                    if($is_active) { $cnt++; $total_due += $due; }
                ?>
                        ...
                        <div class="d-flex align-items-center">
                            ...
                            <div class="ms-3 flex-grow-1 overflow-hidden">
                                ...
                                <div class="d-flex align-items-center gap-2 mt-1">
                                    <span class="badge bg-primary-subtle text-primary rounded-pill px-2" style="font-size: 0.65rem;">Roll: <?php echo $row["rollno"]; ?></span>
